...
image::jump-app-ingress-envoy-gw-sec.png[]
...
image::jump-app-trou-05-padr.png[]
...
[source,input,subs="+macros,+attributes"]
----
curl --header "Host: back-golang-user1-namespace.apps.acidonpe.sandbox37.opentlc.com" back-golang.user1-namespace.svc.cluster.local:8442 -v
----

.HTTP Response _Connection reset by peer_
image::jump-app-trou-05-k8ssrv.png[]
...
=== *Emulate Envoy Proxy connectivity via command line*
...
oc project <istio-system>
oc get pod
oc exec <ingressgateway_pod_id> curl localhost:15000/config_dump > ingress.json
...
oc project istio-system
oc rsh istio-ingressgateway-cfdfb66ff-952hx
...
curl --connect-to back-golang-user1-<user_namespace>.<openshift_apps_domain>:443:back-golang.user1-namespace.svc.cluster.local:8442 https://back-golang-<user_namespace>.<openshift_apps_domain>/ -k -v --cacert ca-cert.pem --cert istio.test.crt --key istio.test.key
