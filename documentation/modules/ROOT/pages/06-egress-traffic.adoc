...
[source,input,subs="+macros,+attributes"]
----
oc -n <user_namespace>-mesh-external rsh <nginx_pod_id>  curl localhost:443 -k -v
----
...
[source,input,subs="+macros,+attributes"]
----
oc -n <user_namespace> exec -it <testing_pod_id> -c sleep -- curl -v --cacert /etc/nginx-ca-certs/example.com.crt --cert /etc/nginx-client-certs/tls.crt --key /etc/nginx-client-certs/tls.key https://my-nginx.<user_namespace>-mesh-external.svc.cluster.local -k
----
...
[source,input,subs="+macros,+attributes"]
----
oc -n <user_namespace> exec -it <testing_pod_id> -c sleep -- curl -v https://my-nginx.<user_namespace>-mesh-external.svc.cluster.local -k
----
...
* Create the CA certificate in a Kubernetes secret

[.console-input]
[source,input,subs="+macros,+attributes"]
----
oc create -n istio-system secret generic <user_namespace>-mesh-external-nginx-ca-certs --from-file=example.com.crt
----

.Nginx service with mTLS
image::jump-app-mesh-external-nginx-istio-system-cert-ca.png[]

* Create the client mTLS certificates in another Kubernetes secret

[.console-input]
[source,input,subs="+macros,+attributes"]
----
oc create -n istio-system secret tls <user_namespace>-mesh-external-nginx-client-certs --key client.example.com.key --cert client.example.com.crt
----

.Nginx service with mTLS
image::jump-app-mesh-external-nginx-istio-system-cert.png[]

* Create a _patch_ file in order to edit the _Egress Gateway_ deployment
...
[{
  ...
  "value": {
    "mountPath": "/etc/istio/<user_namespace>-mesh-external/nginx-client-certs",
    "name": "<user_namespace>-mesh-external-nginx-client-certs",
    "readOnly": true
  }
},
{
  ...
  "name": "<user_namespace>-mesh-external-nginx-client-certs",
    "secret": {
      "secretName": "<user_namespace>-mesh-external-nginx-client-certs",
      "optional": true
    }
  }
},
{
  ...
  "value": {
    "mountPath": "/etc/istio/<user_namespace>-mesh-external/nginx-ca-certs",
    "name": "<user_namespace>-mesh-external-nginx-ca-certs",
    "readOnly": true
  }
},
{
  ...
  "name": "<user_namespace>-mesh-external-nginx-ca-certs",
    "secret": {
      "secretName": "<user_namespace>-mesh-external-nginx-ca-certs",
      "optional": true
    }
  }
}]
...
* Modify the _Egress Gateway_ deployment
...
Once the previous steps have been performed, it is time to test the connection between the _Egress Gateway_ and the _external service_

* Check if the certificates exits

[.console-input]
[source,input,subs="+macros,+attributes"]
----
oc exec -n istio-system "$(oc -n istio-system get pods -l istio=egressgateway -o jsonpath='{.items[0].metadata.name}')" -- ls -alR /etc/istio/<user_namespace>-mesh-external
----

.Nginx service with mTLS
image::jump-app-mesh-external-nginx-client-certs-ok.png[]
...
[source,input,subs="+macros,+attributes"]
----
oc exec -n istio-system "$(oc -n istio-system get pods -l istio=egressgateway -o jsonpath='{.items[0].metadata.name}')" -- curl -v --cacert /etc/istio/<user_namespace>-mesh-external/nginx-ca-certs/example.com.crt --cert /etc/istio/<user_namespace>-mesh-external/nginx-client-certs/tls.crt --key /etc/istio/<user_namespace>-mesh-external/nginx-client-certs/tls.key https://my-nginx.<user_namespace>-mesh-external.svc.cluster.local -k
----
...
[source,input,subs="+macros,+attributes"]
----
oc exec "$(oc get pod -l app=sleep -o jsonpath={.items..metadata.name})" -c sleep -- curl -sS http://my-nginx.<user_namespace>-mesh-external.svc.cluster.local -k -v
----
